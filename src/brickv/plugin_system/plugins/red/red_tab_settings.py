...
class REDTabSettings(QtGui.QWidget, Ui_REDTabSettings):
    ...
    def tab_on_focus(self):
        ...
        self.bdw = self.tbox_settings.widget(1)
        self.bdw.session = self.session
        self.bdw.script_manager = self.script_manager
        
        
        self.dtw = self.tbox_settings.widget(2)
        self.dtw.session = self.session
        self.dtw.script_manager = self.script_manager
        
        self.fsw = self.tbox_settings.widget(3)
        self.fsw.session = self.session
        self.fsw.script_manager = self.script_manager
        ...
        if index == BOX_INDEX_NETWORK:
            ...
            self.bdw.tab_on_focus()
        elif index == BOX_INDEX_DATETIME:
            self.dtw.tab_on_focus()
        elif index == BOX_INDEX_FILESYSTEM:
            self.fsw.tab_on_focus()
    ...
    def tab_off_focus(self):
        ...
        if index == BOX_INDEX_BRICKD:
            self.bdw.tab_off_focus()
        elif index == BOX_INDEX_NETWORK:
            self.network_stat_refresh_timer.stop()
        elif index == BOX_INDEX_DATETIME:
            self.dtw.tab_off_focus()
        elif index == BOX_INDEX_FILESYSTEM:
            self.fsw.tab_off_focus()
