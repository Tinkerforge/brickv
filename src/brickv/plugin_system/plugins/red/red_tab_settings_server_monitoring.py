...
DEFAULT_COL_WIDTH_WARNING  = 400
DEFAULT_COL_WIDTH_CRITICAL = 400
DEFAULT_COL_WIDTH_EMAIL    = 160
DEFAULT_COL_WIDTH_DELETE   = 10
...
class REDTabSettingsServerMonitoring(QtGui.QWidget, Ui_REDTabSettingsServerMonitoring):
    def __init__(self):
        ...
        self.tview_rules.setModel(self.model_rules)
        self.tview_rules.setColumnWidth(0, DEFAULT_COL_WIDTH_NAME)
        self.tview_rules.setColumnWidth(1, DEFAULT_COL_WIDTH_BRICKLET)
        self.tview_rules.setColumnWidth(2, DEFAULT_COL_WIDTH_UID)
        self.tview_rules.setColumnWidth(3, DEFAULT_COL_WIDTH_WARNING)
        self.tview_rules.setColumnWidth(4, DEFAULT_COL_WIDTH_CRITICAL)
        self.tview_rules.setColumnWidth(5, DEFAULT_COL_WIDTH_EMAIL)
        self.tview_rules.setColumnWidth(6, DEFAULT_COL_WIDTH_DELETE)

        # Connecting signals to slots
        self.pbutton_add_rule.clicked.connect(self.slot_pbutton_add_rule_clicked)
        self.pbutton_remove_all_rules.clicked.connect(self.slot_pbutton_remove_all_rules_clicked)
        self.pbutton_refresh.clicked.connect(self.slot_pbutton_refresh_clicked)
        self.pbutton_save.clicked.connect(self.slot_pbutton_save_clicked)
    ...
    def add_new_rule(self):
        ...
        for c in range(0, COUNT_COLUMNS_RULES_MODEL):
            if c == COL_INDEX_NAME:
                ...
                self.tview_rules.setIndexWidget(index, QtGui.QLineEdit())

            elif c == COL_INDEX_BRICKLET:
                print '*** INDEX =', c
                item = self.model_rules.item(r, c)
                index = self.model_rules.indexFromItem(item)
                cbox = QtGui.QComboBox(self.tview_rules)
                cbox.addItems(['PTC', 'Temperature', 'Humidity'])
                print 'CBOX = ', cbox.count()
                self.tview_rules.setIndexWidget(index, cbox)

            elif c == COL_INDEX_UID:
                item = self.model_rules.item(r, c)
                index = self.model_rules.indexFromItem(item)
                self.tview_rules.setIndexWidget(index, QtGui.QComboBox())
                ...
                self.tview_rules.setIndexWidget(index, sss)
                ...
                self.tview_rules.setIndexWidget(index, btn)
    ...
    def update_gui(self, event):
        if event == EVENT_CLICKED_ADD:
            ...
            if self.model_rules.rowCount() != 0:
                self.pbutton_remove_all_rules.setEnabled(True)
            else:
                self.pbutton_remove_all_rules.setEnabled(False)

        elif event == EVENT_CLICKED_REFRESH:
            self.pbutton_refresh.setText('Refreshing...')
            self.sarea_sm.setEnabled(False)

        elif event == EVENT_CLICKED_SAVE:
            self.pbutton_save.setText('Saving...')
            self.sarea_sm.setEnabled(False)

        elif event == EVENT_INPUT_CHANGED:
            self.pbutton_save.setEnabled(True)
    ...
    def update_gui_after_refresh(self):
        self.pbutton_refresh.setText('Refresh')
        self.sarea_sm.setEnabled(True)

    def update_gui_after_save(self, result):
        self.pbutton_save.setText('Save')
        self.sarea_sm.setEnabled(False)

        if result:
            self.pbutton_save.setEnabled(False)
        else:
            self.pbutton_save.setEnabled(True)
    ...
    def slot_delete_rule_clicked(self):
        ...
        for r in range(self.model_rules.rowCount()):
            for c in range(0, COUNT_COLUMNS_RULES_MODEL):
                ...
                if sender == self.tview_rules.indexWidget(index_item):
                    print 'DELETE ROW: ', r
                    self.model_rules.removeRows(r, 1)
                    break

    def slot_pbutton_add_rule_clicked(self):
        print 'slot_pbutton_add_rule_clicked'
        self.update_gui(EVENT_CLICKED_ADD)
        self.update_gui(EVENT_INPUT_CHANGED)

    def slot_pbutton_remove_all_rules_clicked(self):
        print 'slot_pbutton_remove_all_rules_clicked'
        self.update_gui(EVENT_CLICKED_REMOVE_ALL)
        self.update_gui(EVENT_INPUT_CHANGED)

    def slot_pbutton_refresh_clicked(self):
        print 'slot_pbutton_refresh_clicked'
        self.update_gui(EVENT_CLICKED_REFRESH)
        self.update_gui(EVENT_INPUT_CHANGED)
        self.update_gui_after_refresh()

    def slot_pbutton_save_clicked(self):
        print 'slot_pbutton_save_clicked'
