...
gps.py: GPS 3.0 Plugin Implementation
...
from brickv.plugin_system.plugins.gps_v3.ui_gps_v3 import Ui_GPSV3
from brickv.bindings.bricklet_gps_v3 import BrickletGPSV3
from brickv.callback_emulator import CallbackEmulator
from brickv.async_call import async_call

class GPSV3(COMCUPluginBase, Ui_GPSV3):
    qtcb_pps = pyqtSignal()

    def __init__(self, *args):
        COMCUPluginBase.__init__(self, BrickletGPSV3, *args)
        ...
        self.cbe_universal_gps = CallbackEmulator(self,
                                                  self.get_universal_gps,
                                                  None,
                                                  self.cb_universal_gps,
                                                  self.increase_error_count)

        self.cbe_universal_glo = CallbackEmulator(self,
                                                  self.get_universal_glo,
                                                  None,
                                                  self.cb_universal_glo,
                                                  self.increase_error_count)

        self.cbe_universal_gal = CallbackEmulator(self,
                                                  self.get_universal_gal,
                                                  None,
                                                  self.cb_universal_gal,
                                                  self.increase_error_count)
        ...
        self.gal_model = QStandardItemModel(32, 3, self)
        self.gal_table.setModel(self.gal_model)
        self.gal_model.setHorizontalHeaderItem(0, QStandardItem('Elevation (°)'))
        self.gal_model.setHorizontalHeaderItem(1, QStandardItem('Azimuth (°)'))
        self.gal_model.setHorizontalHeaderItem(2, QStandardItem('SNR (dB)'))
        for i in range(32):
            self.gal_model.setVerticalHeaderItem(i, QStandardItem('Sat ' + str(i+1+300)))
        self.gal_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
    
        self.fix_led_off_action = QAction('Off', self)
        self.fix_led_off_action.triggered.connect(lambda: self.gps.set_fix_led_config(BrickletGPSV3.FIX_LED_CONFIG_OFF))
        self.fix_led_on_action = QAction('On', self)
        self.fix_led_on_action.triggered.connect(lambda: self.gps.set_fix_led_config(BrickletGPSV3.FIX_LED_CONFIG_ON))
        self.fix_led_show_heartbeat_action = QAction('Show Heartbeat', self)
        self.fix_led_show_heartbeat_action.triggered.connect(lambda: self.gps.set_fix_led_config(BrickletGPSV3.FIX_LED_CONFIG_SHOW_HEARTBEAT))
        self.fix_led_show_fix_action = QAction('Show Fix', self)
        self.fix_led_show_fix_action.triggered.connect(lambda: self.gps.set_fix_led_config(BrickletGPSV3.FIX_LED_CONFIG_SHOW_FIX))
        self.fix_led_show_pps_action = QAction('Show PPS', self)
        self.fix_led_show_pps_action.triggered.connect(lambda: self.gps.set_fix_led_config(BrickletGPSV3.FIX_LED_CONFIG_SHOW_PPS))
    ...
    def get_universal_gal(self):
        counter = self.gal_counter
        self.gal_counter = (self.gal_counter + 1) % 33

        if counter == 0:
            return counter, self.gps.get_satellite_system_status(self.gps.SATELLITE_SYSTEM_GALILEO)
        else:
            return counter, self.gps.get_satellite_status(self.gps.SATELLITE_SYSTEM_GALILEO, counter)
    ...
    def cb_universal_gal(self, data):
        try:
            counter, x = data
            if counter == 0:
    ...
    def restart_clicked(self, restart_type):
        if restart_type > 0:
            ...
            for i in range(32):
                ...
                self.gal_model.setItem(i, 0, QStandardItem('0'))
                self.gal_model.setItem(i, 1, QStandardItem('0'))
                self.gal_model.setItem(i, 2, QStandardItem('0'))
    ...
    def get_fix_led_config_async(self, config):
        if config == BrickletGPSV3.FIX_LED_CONFIG_OFF:
            self.fix_led_off_action.trigger()
        elif config == BrickletGPSV3.FIX_LED_CONFIG_ON:
            self.fix_led_on_action.trigger()
        elif config == BrickletGPSV3.FIX_LED_CONFIG_SHOW_HEARTBEAT:
            self.fix_led_show_heartbeat_action.trigger()
        elif config == BrickletGPSV3.FIX_LED_CONFIG_SHOW_FIX:
    ...
    def has_device_identifier(device_identifier):
        return device_identifier == BrickletGPSV3.DEVICE_IDENTIFIER
