...
silent_stepper_v2.py: Silent Stepper Bricklet 2.0 Plugin implementation
...
from brickv.plugin_system.plugins.silent_stepper_v2.ui_silent_stepper_v2 import Ui_SilentStepperV2
...
class SilentStepperV2(COMCUPluginBase, Ui_SilentStepperV2):
    ...
    def __init__(self, *args):
        ...
        self.silent_stepper_v2.register_callback(self.silent_stepper_v2.CALLBACK_POSITION_REACHED,
                                              self.qtcb_position_reached.emit)

        self.qtcb_under_voltage.connect(self.cb_under_voltage)
        self.silent_stepper_v2.register_callback(self.silent_stepper_v2.CALLBACK_UNDER_VOLTAGE,
    ...
    def step_configuration_changed(self, _):
        ...
        try:
            self.silent_stepper_v2.set_step_configuration(step_resolution, interpolation)
    ...
    def basic_configuration_changed(self, _):
        ...
        try:
            self.silent_stepper_v2.set_basic_configuration(standstill_current, motor_run_current, standstill_delay_time, power_down_time, stealth_threshold, coolstep_threshold, classic_threshold, high_velocity_chopper_mode)
        except ip_connection.Error:
            return
    ...
    def spreadcycle_configuration_changed(self, _):
        ...
        try:
            self.silent_stepper_v2.set_spreadcycle_configuration(slow_decay_duration, enable_random_slow_decay, fast_decay_duration, hysteresis_start_value, hysteresis_end_value, sine_wave_offset, chopper_mode, comparator_blank_time, fast_decay_without_comparator)
        except ip_connection.Error:
            return
    ...
    def stealth_configuration_changed(self, _):
        ...
        try:
            self.silent_stepper_v2.set_stealth_configuration(enable_stealth, amplitude, gradient, enable_autoscale, force_symmetric, freewheel_mode)
        except ip_connection.Error:
            return
    ...
    def coolstep_configuration_changed(self, _):
        ...
        try:
            self.silent_stepper_v2.set_coolstep_configuration(minimum_stallguard_value, maximum_stallguard_value, current_up_step_width, current_down_step_width, minimum_current, stallguard_threshold_value, stallguard_mode)
        except ip_connection.Error:
            return
    ...
    def misc_configuration_changed(self, _):
        ...
        try:
            self.silent_stepper_v2.set_misc_configuration(disable_short_to_ground_protection, synchronize_phase_frequency)
        except ip_connection.Error:
            return
    ...
    def motor_current_button_clicked(self):
        ...
        qid.setIntStep(100)
        async_call(self.silent_stepper_v2.get_motor_current, None, qid.setIntValue, self.increase_error_count)
    ...
    def minimum_motor_voltage_button_clicked(self):
        ...
        qid.setIntStep(100)
        async_call(self.silent_stepper_v2.get_minimum_voltage, None, qid.setIntValue, self.increase_error_count)
    ...
    def update_start(self):
        async_call(self.silent_stepper_v2.get_max_velocity, None, self.get_max_velocity_async, self.increase_error_count)
        async_call(self.silent_stepper_v2.get_speed_ramping, None, self.get_speed_ramping_async, self.increase_error_count)
        async_call(self.silent_stepper_v2.get_enabled, None, self.get_enabled_async, self.increase_error_count)
        async_call(self.silent_stepper_v2.get_step_configuration, None, self.get_step_configuration_async, self.increase_error_count)
        async_call(self.silent_stepper_v2.get_basic_configuration, None, self.get_basic_configuration_async, self.increase_error_count)
        async_call(self.silent_stepper_v2.get_spreadcycle_configuration, None, self.get_spreadcycle_configuration_async, self.increase_error_count)
        async_call(self.silent_stepper_v2.get_stealth_configuration, None, self.get_stealth_configuration_async, self.increase_error_count)
        async_call(self.silent_stepper_v2.get_coolstep_configuration, None, self.get_coolstep_configuration_async, self.increase_error_count)
        async_call(self.silent_stepper_v2.get_misc_configuration, None, self.get_misc_configuration_async, self.increase_error_count)

    def update_data(self):
        async_call(self.silent_stepper_v2.get_remaining_steps, None, self.remaining_steps_update, self.increase_error_count)
        async_call(self.silent_stepper_v2.get_current_position, None, self.position_update, self.increase_error_count)
        async_call(self.silent_stepper_v2.get_current_velocity, None, self.speedometer.set_velocity, self.increase_error_count)
        async_call(self.silent_stepper_v2.get_gpio_state, None, self.gpio_state_update, self.increase_error_count)

        self.update_counter += 1
        if self.update_counter % 10 == 0:
            async_call(self.silent_stepper_v2.get_motor_current, None, self.maximum_current_update, self.increase_error_count)
            async_call(self.silent_stepper_v2.get_input_voltage, None, self.input_voltage_update, self.increase_error_count)
            async_call(self.silent_stepper_v2.get_minimum_voltage, None, self.minimum_voltage_update, self.increase_error_count)
            async_call(self.silent_stepper_v2.get_driver_status, None, self.driver_status_update, self.increase_error_count)
    ...
    def gpio_action_changed(self, channel):
        ...
        if rising  == 1: action = action | self.silent_stepper_v2.GPIO_ACTION_NORMAL_STOP_RISING_EDGE
        if falling == 1: action = action | self.silent_stepper_v2.GPIO_ACTION_NORMAL_STOP_FALLING_EDGE
        if rising  == 2: action = action | self.silent_stepper_v2.GPIO_ACTION_FULL_BRAKE_RISING_EDGE
        if falling == 2: action = action | self.silent_stepper_v2.GPIO_ACTION_FULL_BRAKE_FALLING_EDGE
    ...
    def gpio_configuration_changed(self, channel):
        ...
        self.silent_stepper_v2.set_gpio_configuration(channel, debounce, stop_deceleration)
